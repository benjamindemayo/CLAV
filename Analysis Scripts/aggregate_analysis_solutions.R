...
hist(data$age)
hist(data$age,breaks=2:5)
hist(data$age,breaks=c(seq(2,5,.5)))
...
# use this function and aggregate subids
...
mss <- aggregate(correct ~ age.group + condition + subid,data,mean)

# THEN aggregate sem across subjects 
ms <- aggregate(correct ~  age.group + condition,mss,mean)
ms$err <- aggregate(correct ~ age.group + condition,mss,sem)$correct
